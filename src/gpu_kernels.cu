...
namespace gansu::gpu
{
    ...
    __global__ void computeUnifiedFockMatrix_ROHF_kernel(const real_t *d_fock_mo_closed_matrix, const real_t *d_fock_mo_open_matrix, const ROHF_ParameterSet rohf_params, real_t *d_unified_fock_matrix, const int num_closed, const int num_open, const size_t num_basis)
    {
        ...
        if (i < num_closed)
            shell_i = CLOSED;
        else if (i < num_closed + num_open)
            shell_i = OPEN;
        else
            shell_i = VIRTUAL;
        if (j < num_closed)
            shell_j = CLOSED;
        else if (j < num_closed + num_open)
            shell_j = OPEN;
        else
            shell_j = VIRTUAL;
        ...
        if (i != j)
            d_unified_fock_matrix[j * num_basis + i] = d;
    }
    ...
    __global__ void computeRIIntermediateMatrixB_kernel(const double *d_three_center_eri, const double *d_matrix_L, double *d_matrix_B, const int num_basis, const int num_auxiliary_basis)
    {
        ...
        if (id >= num_auxiliary_basis * num_basis * num_basis)
            return;
    ...
    inline __device__ void swap_indices(int &mu, int &nu)
    {
        int tmp = mu;
        mu = nu;
        nu = tmp;
    ...
    inline __device__ int utm_id(int mu, int nu)
    {
        return (mu <= nu) ? nu * (nu + 1) / 2 + mu : mu * (mu + 1) / 2 + nu;
    }

    inline __device__ int twoDim2oneDim(int mu, int nu, int num_basis)
    {
        return (mu < nu) ? num_basis * mu + nu : num_basis * nu + mu;
    }

    inline __device__ void add2J(double val, double *g_J,
                                 int mu, int nu, int la, int si, int num_basis, const double *g_dens)
    {
        ...
        else if (is_sym_bra)
        {
            ...
            atomicAdd(g_J + num_basis * la + si, 1.0 * g_dens[num_basis * mu + nu] * val);
            atomicAdd(g_J + num_basis * si + la, 1.0 * g_dens[num_basis * mu + nu] * val);
        ...
        {
            atomicAdd(g_J + num_basis * mu + nu, 1.0 * g_dens[num_basis * la + si] * val);
            atomicAdd(g_J + num_basis * nu + mu, 1.0 * g_dens[num_basis * la + si] * val);
        ...
        {
            atomicAdd(g_J + num_basis * mu + nu, 2.0 * g_dens[num_basis * la + si] * val);
            atomicAdd(g_J + num_basis * nu + mu, 2.0 * g_dens[num_basis * la + si] * val);
            // atomicAdd(g_J + num_basis * mu + la, (-0.5) * g_dens[num_basis * nu + si] * val);
            // atomicAdd(g_J + num_basis * nu + si, (-0.5) * g_dens[num_basis * mu + la] * val);
        ...
        else
        {
            ...
            atomicAdd(g_J + num_basis * nu + mu, 2.0 * g_dens[num_basis * la + si] * val);
            atomicAdd(g_J + num_basis * la + si, 2.0 * g_dens[num_basis * mu + nu] * val);
            atomicAdd(g_J + num_basis * si + la, 2.0 * g_dens[num_basis * mu + nu] * val);
            // atomicAdd(g_J + num_basis * mu + la, ((mu == la) ? -1.0 : -0.5) * g_dens[num_basis * nu + si] * val);
            // atomicAdd(g_J + ((nu <= si) ? num_basis * nu + si : num_basis * si + nu), ((nu == si) ? -1.0 : -0.5) * g_dens[num_basis * mu + la] * val);
            // atomicAdd(g_J + num_basis * mu + si, (-0.5) * g_dens[num_basis * nu + la] * val);
    ...
    inline __device__ void add2K(double val, double *g_K,
                                 int mu, int nu, int la, int si, int num_basis, const double *g_dens)
    {
        if (mu > nu)
        {
            ...
            for (int lmn_b = 0; lmn_b < comb_max(orbital_B); lmn_b++)
            {
                ...
                for (int lmn_c = 0; lmn_c < comb_max(orbital_C); lmn_c++)
                {
                    ...
                    for (int lmn_d = 0; lmn_d < comb_max(orbital_D); lmn_d++)
                    {
                        ...
                        if (orbital_A == orbital_C && orbital_C == 0 && orbital_B == orbital_D && orbital_D == 1 && (size_a == size_c && size_b == size_d) && lmn_b > lmn_d)
                            continue; // spsp
                        if (orbital_A == 0 && orbital_C == orbital_D && orbital_D == 1 && size_c == size_d && lmn_c > lmn_d)
                            continue; // sspp, sppp
                        if (orbital_A == orbital_B && orbital_B == orbital_C && orbital_C == orbital_D && orbital_D == 1)
                        { //
                            if (size_c == size_d && lmn_c > lmn_d)
                                continue;
                            if (size_a == size_b && lmn_a > lmn_b)
                                continue;
                        }
                        ...
                        thread_val *= Norm * g_cgto_normalization_factors[size_a + lmn_a] * g_cgto_normalization_factors[size_b + lmn_b] * g_cgto_normalization_factors[size_c + lmn_c] * g_cgto_normalization_factors[size_d + lmn_d];

                        if (!is_bra_symmetric && size_a == size_b)
                            thread_val *= 2.0;
                        if (!is_ket_symmetric && size_c == size_d)
                            thread_val *= 2.0;
                        if (!is_braket_symmetric && (size_a == size_c && size_b == size_d))
                            thread_val *= 2.0;
                        if (orbital_A == orbital_B && orbital_B == orbital_C && orbital_C == orbital_D && orbital_D == 1 && (utm_id(size_a, size_b) == utm_id(size_c, size_d)) && twoDim2oneDim(size_a + lmn_a, size_b + lmn_b, num_basis) != twoDim2oneDim(size_c + lmn_c, size_d + lmn_d, num_basis))
                            thread_val *= 0.5;
