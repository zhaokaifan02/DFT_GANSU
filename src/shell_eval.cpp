    ...
    std::vector<int>& ao_loc)
{
    ...
    const int natm = (int)atoms.size();
    atm.resize(natm * ATM_SLOTS);

    // env 布局：先放所有坐标（每原子 3 个 double），后续 append 指数/系数块
    auto push_env_block = [&](const std::vector<double>& v){
        int ptr = (int)env.size();
        env.insert(env.end(), v.begin(), v.end());
        return ptr; // 返回 env 偏移
    };

    // 1) 坐标入 env，并填 atm 槽位
    env.reserve(natm * 3);
    for (int A = 0; A < natm; ++A){
        int coord_ptr = (int)env.size();
        env.push_back(atoms[A].x);
        env.push_back(atoms[A].y);
        env.push_back(atoms[A].z);

        atm[A*ATM_SLOTS + CHARGE_OF]      = atoms[A].Z;
        atm[A*ATM_SLOTS + PTR_COORD]      = coord_ptr;
        atm[A*ATM_SLOTS + NUC_MOD_OF]     = 0;       // 点核
        atm[A*ATM_SLOTS + PTR_ZETA]       = 0;       // 未用
        atm[A*ATM_SLOTS + PTR_FRAC_CHARGE]= 0;       // 未用
        atm[A*ATM_SLOTS + RESERVE_ATMSLOT]= 0;
    }
    ...
    // 元素名列表来自 BasisSet；给每个原子匹配自身元素
    auto Z2name = [&](int Z){ return ATOMIC_NUMBER_MINUS_ONE_TO_ELEMENT_NAME[Z-1]; }; // :contentReference[oaicite:3]{index=3}

    for (int A = 0; A < natm; ++A){
        std::string ename = Z2name(atoms[A].Z);
        const ElementBasisSet& ebs = bs.get_element_basis_set(ename); // :contentReference[oaicite:4]{index=4}

        for (size_t ig = 0; ig < ebs.get_num_contracted_gausses(); ++ig){
            const ContractedGauss& cg = ebs.get_contracted_gauss(ig); // :contentReference[oaicite:5]{index=5}
            int l = SHELL_NAME_TO_SHELL_TYPE.at(cg.get_type());       // s=0,p=1,... :contentReference[oaicite:6]{index=6}

            // indices→env：把该壳的所有 exponent / coefficient 追加到 env
            ...
            for (size_t ip = 0; ip < cg.get_num_primitives(); ++ip){
                const PrimitiveGauss& pg = cg.get_primitive_gauss(ip); // :contentReference[oaicite:7]{index=7}
                exps.push_back(pg.exponent);
                coeffs.push_back(pg.coefficient);
            }
            int ptr_exp   = push_env_block(exps);
            int ptr_coeff = push_env_block(coeffs);

            // bas 行（一个壳一行；SP 已在读入时拆为 S 与 P 两行）:contentReference[oaicite:8]{index=8}
            int rec[BAS_SLOTS];
            rec[ATOM_OF]    = A;
            rec[ANG_OF]     = l;
            rec[NPRIM_OF]   = (int)cg.get_num_primitives();
            rec[NCTR_OF]    = 1;      // 一个收缩
            rec[KAPPA_OF]   = 0;      // 笛卡尔
            rec[PTR_EXP]    = ptr_exp;
            rec[PTR_COEFF]  = ptr_coeff;
            rec[RESERVE_BASLOT] = 0;

            bas.insert(bas.end(), rec, rec + BAS_SLOTS);

            // 更新 ao_loc（笛卡尔：deg=(l+1)(l+2)/2）:contentReference[oaicite:9]{index=9}
            int deg = (l + 1) * (l + 2) / 2;
            ao_loc.push_back(ao_loc.back() + deg);
        }
    }

    // 注意：如果后续还要支持多重中心 nctr>1 的壳/收缩并列，只需把多列系数写入 env，
    //       rec[NCTR_OF] 改为真实 nctr，并相应扩展 ptr_coeff 的存储布局即可
}
