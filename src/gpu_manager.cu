...
namespace gansu::gpu
{
    ...
    {
        if (size > 1024)
        { // 1024 is the maximum number of threads per block. If the size is larger than 1024, two or more blocks are required.
     ...
     */
    void solve_lower_triangular(double *d_A, double *d_B, int row, int col, bool if_trans_A = false)
    {
        cublasHandle_t cublasHandle = GPUHandle::cublas();
        ...
        cublasDtrsm(
            ...
            (if_trans_A) ? CUBLAS_OP_T : CUBLAS_OP_N,
    ...
    void computeJMatrix_DFT_RHF(const real_t *d_density_matrix, real_t *d_J_matrix,
                                const std::vector<ShellTypeInfo> &shell_type_infos,
                                const std::vector<ShellPairTypeInfo> &shell_pair_type_infos,
                                const PrimitiveShell *d_primitive_shells,
                                const real_t *d_cgto_normalization_factors,
                                ...
                                const bool verbose)
    {
        ...
        for (const auto &quadruple : shell_quadruples)
        {
            ...
            gpu::MD_1T1SP_Direct_J<<<num_blocks, threads_per_block, 0, streams[stream_id++]>>>(d_J_matrix,
                                                                                               d_density_matrix,
                                                                                               d_primitive_shells,
                                                                                               d_primitive_shell_pair_indices,
                                                                                               d_cgto_normalization_factors,
    ...
    void computeKMatrix_DFT_RHF(const real_t *d_density_matrix, real_t *d_K_matrix,
                                const std::vector<ShellTypeInfo> &shell_type_infos,
                                const std::vector<ShellPairTypeInfo> &shell_pair_type_infos,
                                const PrimitiveShell *d_primitive_shells,
                                const real_t *d_cgto_normalization_factors,
                                ...
                                const bool verbose)
    {
        ...
        for (const auto &quadruple : shell_quadruples)
        {
            ...
            gpu::MD_1T1SP_Direct_K<<<num_blocks, threads_per_block, 0, streams[stream_id++]>>>(d_K_matrix,
                                                                                               d_density_matrix,
                                                                                               d_primitive_shells,
                                                                                               d_primitive_shell_pair_indices,
                                                                                               d_cgto_normalization_factors,
    ...
    {
        // grid.pyのinit_build()を、PySCFやテキスト書込み/読み出しを用いないように実装するイメージです
        // もしこのやり方で不都合がある場合は教えてください！
        //
        // dft_GenGrid.cppのdft_gen_ao(), dft_gen_grid()を実装して、aoGridsおよびgridsを更新するイメージでいます！
        ...
        {
            std::array<double, 3> atom = {h_atoms[i].coordinate.x,
                                          h_atoms[i].coordinate.y,
                                          h_atoms[i].coordinate.z};
            atm_coords.push_back(atom);
            charges.push_back(h_atoms[i].atomic_number);
        }
        // atom_basis
    ...
    void build_vxc_matrix(const int nao, const int ngrids, const double *d_ao, std::vector<double> &weights_vector, double *d_rho, double *d_V)
    {
        ...
        const size_t left_byte = (SAFE_FREE > aux_buf) ?
                                (SAFE_FREE - aux_buf) : 0;
        if (left_byte == 0)
