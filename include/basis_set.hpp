...
namespace gansu
{
    ...
    public:
         ...
         * @brief
         */
        const std::vector<PrimitiveGauss> &get_primitives() const noexcept
        {
            return primitives;
        }
        /**
        ...
        std::vector<real_t> get_normalization_factor() const
        {
            ...
            for (const auto &angular_momentums : angular_momentum_list)
            {
                ...
                {
                    real_t n_a = std::pow(2.0 * M_PI, -3.0 / 4.0) * std::pow(4.0 * primitive_a.exponent, L / 2.0 + 0.75);

                    for (const auto &primitive_b : primitives)
                    {
                        real_t n_b = std::pow(2.0 * M_PI, -3.0 / 4.0) * std::pow(4.0 * primitive_b.exponent, L / 2 + 0.75);

                        normalize_factor += primitive_a.coefficient * n_a * primitive_b.coefficient * n_b / std::pow(2.0 * (primitive_a.exponent + primitive_b.exponent), L + 1.5);
                    }
                }
    ...
    public:
        ...
        /**
         * @author fan DFT-Grid
         * @brief get All shells
         * @return
         */
        const std::vector<ContractedGauss> &get_contracted_gaussians() const noexcept
        {
            return contracted_gausses;
        }
        /**
